...
interface MagicTextRevealProps {
  ...
  /**
   * Optional external hover control. When provided, the component's
   * own mouseenter/leave handlers are ignored and this prop drives the
   * particle-to-text reveal. Lets a parent group multiple instances
   * into a single hover unit.
   */
...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  // If a parent has supplied `forceHovered`, that wins. Otherwise the
  // component manages its own hover state from local mouse events.
  const isHovered = forceHovered !== undefined ? forceHovered : internalHovered;
  ...
  // Event handlers — used only when no external `forceHovered` was passed.
  const handleMouseEnter = useCallback(() => {
    if (forceHovered !== undefined) return;
  ...
  }, [forceHovered, resetOnMouseLeave, hasBeenShown]);
  return (
    <div
      ref={wrapperRef}
